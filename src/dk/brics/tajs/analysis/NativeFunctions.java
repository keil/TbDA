...
/**
 * Dispatch evaluation of native functions
 * and common functionality used by the native functions.
 */
public class NativeFunctions {
	...
	public static void expectParameters(NativeObject nativeobject, CallInfo call, Solver.SolverInterface c, int min, int max) {
		...
		if (max != -1) {
			c.addMessage(num_actuals_unknown ? Status.MAYBE : num_actuals > max ? Status.CERTAIN : Status.NONE,
					Severity.HIGH, "Too many parameters to native function " + nativeobject);
			// TODO: implementations *may* throw TypeError if too many parameters to native functions (p.76)
		}
	}

	/**
	 * Reads the value of a call parameter.
	 * Returns 'undefined' if too few parameters.
	 * The first parameter has number 0.
	 */
	 ...
	 * Also models truncation of the array if the 'length' property is being set.
	 * Sets the state to bottom if an exception is definitely thrown.
	 */
	public static void updateArrayLength(Node node, State state, Set<ObjectLabel> objlabels, Value propertyval, Value value, Solver.SolverInterface c) {
		...
		if (definitely_length || maybe_length) {
			...
			// TODO: model array truncation more precisely for constant index values? (like old impl)
			state.deleteUnknownProperty(arrays); // be conservative; could be 'length' being set to 0
			state.writeSpecialProperty(arrays, "length", Value.makeAnyNumUInt(dependency).setAttributes(true, true, false));
			return; // TODO: make sure maybe_length and maybe_index can both be active
		}
		...
		if (definitely_index || maybe_index) {
			// FIXME: model assignment to array index more precisely for constant index values? (micro/testFunctionApply.js)
			state.writeSpecialProperty(arrays, "length", Value.makeAnyNumUInt(dependency).setAttributes(true, true, false));
		}
	}
	 ...
	 * @return true if the exception is definitely thrown
	 */
	public static boolean throwTypeErrorIfWrongKindOfThis(ECMAScriptObjects nativeobject, CallInfo call, State state,
		...
		if (some_bad)
			Exceptions.throwTypeError(state, c);
		c.addMessage(call.getSourceNode(), some_bad ? some_good ? Status.MAYBE : Status.CERTAIN : Status.NONE,
				Severity.HIGH, "TypeError, native function " + nativeobject + " called on invalid object kind, expected " + kind);
