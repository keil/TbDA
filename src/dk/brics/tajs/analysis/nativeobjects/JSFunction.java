...
public class JSFunction {
	...
	public static Value evaluate(ECMAScriptObjects nativeobject, final CallInfo call, final State state, final Solver.SolverInterface c) {
		...
		case FUNCTION: { // 15.3.1 / 15.3.2 (no difference between function and constructor)
			return Value.makeUndef(new Dependency()); // XXX throw new RuntimeException("Don't know how to handle call to 'Function' :-("); // TODO: call to 'Function' (just issue an error message and return undefined?)
		}
		...
		case FUNCTION_APPLY: { // 15.3.4.3
			...
			if (maybe_ok || maybe_typeerror)
				c.addMessage(maybe_typeerror ? maybe_ok ? Status.MAYBE : Status.CERTAIN : Status.NONE, 
		...
		case FUNCTION_CALL: { // 15.3.4.4
			...
			return Value.makeBottom(dependency); // no direct flow to the successor
		}
