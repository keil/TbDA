...
public class HTMLScriptElement {
	...
	public static void build(State s) {
		...
		createDOMProperty(s, DOMWindow.WINDOW, "HTMLScriptElement", Value.makeObject(CONSTRUCTOR, new Dependency(), new DependencyGraphReference()));
		...
		createDOMProperty(s, INSTANCES, "htmlFor", Value.makeAnyStr(new Dependency(), new DependencyGraphReference()), DOMSpec.LEVEL_1);
		createDOMProperty(s, INSTANCES, "event", Value.makeAnyStr(new Dependency(), new DependencyGraphReference()), DOMSpec.LEVEL_1);
		createDOMProperty(s, INSTANCES, "charset", Value.makeAnyStr(new Dependency(), new DependencyGraphReference()), DOMSpec.LEVEL_1);
		createDOMProperty(s, INSTANCES, "defer", Value.makeAnyBool(new Dependency(), new DependencyGraphReference()), DOMSpec.LEVEL_1);
