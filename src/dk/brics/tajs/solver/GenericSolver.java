...
public class GenericSolver<BlockStateType extends IBlockState<BlockStateType>, CallContextType extends ICallContext, StatisticsType extends IStatistics> {
	...
	public class SolverInterface {
		...
		public CallContextType getCurrentContext() {
			if (current_context == null)
				throw new RuntimeException(
						"Unexpected call to getCurrentContext");
		 ...
		 * done. If the message already exists, the status is joined. Uses the
		 * message as key.
		 */
		 ...
		 */
		public void addMessage(Status s, Severity severity, String key,
				String msg) {
		 ...
		 * done. If the message already exists, the status is joined.
		 */
		public void addMessage(Node n, Status s, Severity severity, String key,
				String msg) {
			if (messages_enabled) {
				boolean log;
				...
				if (log && Options.isDebugEnabled())
					System.out.println("addMessage: " + m.getStatus() + " "
							+ m.getNode().getSourceLocation() + ": "
							+ m.getMessage());
			}
		}

		/**
		 * Records the name, type, and line number for a variable. If not in
		 * scan phase, nothing is done.
		 */
		public void record(String variableName, SourceLocation sourceLocation,
				Value value) {
			if (messages_enabled) {
		 ...
		 * given context.
		 */
		public Set<NodeAndContext<CallContextType>> getCallSources(Function f,
				CallContextType callee_context) {
			return the_analysis_lattice_element.getCallGraph().getSources(f,
					callee_context);
		}
		...
		public BlockStateType getCallEdgeState(Node caller,
			...
			return the_analysis_lattice_element.getCallGraph()
					.getCallEdgeState(caller, caller_context, callee);
		}
		 ...
		 */
		public void joinBlockEntry(BlockStateType s, BasicBlock b,
				CallContextType c) {
			joinBlockEntry(s, b, c, false);
		}

		private void joinBlockEntry(BlockStateType s, BasicBlock b,
				CallContextType c, boolean reduce) {
			if (messages_enabled)
				return;
			IAnalysisLatticeElement.MergeResult res = the_analysis_lattice_element
					.joinBlockEntry(s, b, c, reduce);
			if (res != null) {
				worklist.add(worklist.new Entry(b, c, res.getDelta()));
				analysis.getStatistics().registerNewFlow(b, c, res.getDiff());
				if (Options.isDebugEnabled())
					System.out.println("New flow at block "
							+ b.getIndex()
							+ " node "
							+ b.getFirstNode().getIndex()
							+ ", context "
							+ c
							+ (res.getDiff() != null ? ", diff:"
									+ res.getDiff() : ""));
			}
		}
		 ...
		 * operation. Ignored if in scan phase.
		 */
		public void joinFunctionEntry(BlockStateType call_state,
				Node call_node, CallContextType caller_context,
				Function callee, CallContextType callee_context) {
			if (messages_enabled)
				return;
			if (setCallEdge(call_node, callee, caller_context, callee_context,
					call_state)) {
		 ...
		 */
		private boolean setCallEdge(Node call_node, Function callee,
				CallContextType caller_context, CallContextType callee_context,
				BlockStateType call_state) {
			if (the_analysis_lattice_element.getCallGraph().addTarget(
					call_node, callee, caller_context, callee_context,
					call_state)) {
				// process existing ordinary/exceptional return flow
				StoredCurrentContextStateNode current = new StoredCurrentContextStateNode(
						callee_context, null, null);
				analysis.getNodeTransferFunctions().transferReturn(call_node,
						callee, caller_context, callee_context);
				current.restore();
		...
		private class StoredCurrentContextStateNode {
			...
			StoredCurrentContextStateNode(CallContextType c, BlockStateType s,
					Node n) {
				cct = current_context;
				bst = current_state;
				this.n = current_node;
		 ...
		 */
		public BlockStateType getCallState(Node n,
				CallContextType caller_context) {
			return the_analysis_lattice_element.getStates(n.getBlock()).get(
					caller_context);
		}
		...
		public void evaluateGetter(NativeObject nativeobject,
				ObjectLabel objlabel, String propertyname) {
			analysis.evaluateGetter(nativeobject, objlabel, propertyname, this);
		}

		public void evaluateSetter(NativeObject nativeobject,
				ObjectLabel objlabel, String propertyname, Value v) {
			analysis.evaluateSetter(nativeobject, objlabel, propertyname, v,
					this);
		}
	}
	...
	public GenericSolver(
			IAnalysis<BlockStateType, CallContextType, StatisticsType> analysis) {
	 ...
	 */
	public void init(FlowGraph flowgraph, DependencyGraph dependencyGraph) {
	...
	public void solve() {
		...
		block_loop: while (!worklist.isEmpty()) {
			...
			BlockStateType state = the_analysis_lattice_element.getState(block,
					p.getContext());
			if (Options.isDebugEnabled()) {
				System.out
						.println("\nSelecting worklist entry for block "
								+ block.getIndex() + " at "
				...
				System.out
						.println("block "
								...
								+ block.getSourceLocation()
								+ ", transfers: "
								+ String.format("%4d", (analysis
										.getStatistics()
										.getTotalNumberOfNodeTransfers() + 1))
								+ " (avg/node: "
								+ ((float) ((analysis.getStatistics()
										.getTotalNumberOfNodeTransfers() + 1) * 1000 / flowgraph
										.getNumberOfNodes()))
								/ 1000
								+ ")"
								+ ", pending: "
			...
			if (Options.isIntermediateStatesEnabled())
				System.out.println("Before block transfer: "
						+ current_state.toString());
			for (Node n : block.getNodes()) {
				current_node = n;
				if (Options.isDebugEnabled())
					System.out.println("Visiting node "
							+ current_node.getIndex() + ": " + current_node
							+ " at " + current_node.getSourceLocation());
				...
				if (Options.isIntermediateStatesEnabled()
					...
					System.out.println("After node transfer: "
			...
			if (!(first instanceof CallNode) && !(first instanceof ReturnNode)
				...
				analysis.getBlockTransferFunction().transfer(block,
						current_state);
				// edge transfer
				for (Iterator<BasicBlock> i = block.getSuccessors().iterator(); i
						.hasNext();) {
					BasicBlock succ = i.next();
					if (analysis.getEdgeTransferFunctions().transfer(block,
							succ, current_state))
						c.joinBlockEntry(i.hasNext() ? current_state.clone()
								: current_state, succ, current_context);
				}
			}
		}
	...
	public void scan() {
		...
		for (Function function : flowgraph.getFunctions()) {
			...
			for (BasicBlock block : function.getBlocks()) {
				...
				block_loop: for (Map.Entry<CallContextType, BlockStateType> me : the_analysis_lattice_element
						.getStateMapEntries(block)) {
				...
				if (block_is_alive)
					function_is_alive = true;
						...
						&& block != function.getOrdinaryExit()
						&& block != function.getEntry()
						&& !(block.getFirstNode() instanceof CallNode)
						&& !flowgraph.getIgnorableNodes().contains(
								block.getFirstNode()))
					addUnreachableCodeMessage(block.getFirstNode(), "code");
			}
			if (!function_is_alive)
				addUnreachableCodeMessage(
						function.getEntry().getFirstNode(),
						"function"
	...
	private void addUnreachableCodeMessage(Node n, String kind) {
		if (Options.isDebugEnabled())
			System.out.println("Unreachable code at node " + n.getIndex()
					+ ": " + n);
		if (!Options.isUnreachableDisabled()) {
			Message m = new Message(n, Status.CERTAIN, "Unreachable " + kind,
	...
	public List<Message> getMessages() {
		if (messages == null)
			throw new IllegalStateException(
					"getMessages() called before scan()");
		List<Message> es = newList();
		for (Message m : messages.values())
			if (m.getStatus() != Status.NONE
					&& !(m.getSeverity() == Severity.MEDIUM_IF_CERTAIN_NONE_OTHERWISE && m
							.getStatus() != Status.CERTAIN))
				es.add(m);
		Collections.sort(es);
		return es;
	}
