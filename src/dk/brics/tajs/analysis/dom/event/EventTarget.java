...
public class EventTarget {
	...
	public static void build(State s) {
		 ...
		 * Functions.
		 */
		createDOMFunction(s, DOMNode.PROTOTYPE, DOMObjects.EVENT_TARGET_ADD_EVENT_LISTENER, "addEventListener", 3, DOMSpec.LEVEL_2);
		createDOMFunction(s, DOMNode.PROTOTYPE, DOMObjects.EVENT_TARGET_REMOVE_EVENT_LISTENER, "removeEventListener", 3, DOMSpec.LEVEL_2);
		createDOMFunction(s, DOMNode.PROTOTYPE, DOMObjects.EVENT_TARGET_DISPATCH_EVENT, "dispatchEvent", 1, DOMSpec.LEVEL_2);
	...
	public static Value evaluate(DOMObjects nativeObject, FunctionCalls.CallInfo<? extends Node> call, State s, Solver.SolverInterface c) {
		switch (nativeObject) {
		/*
		 * Events added with useCapture = true must be removed separately from
		 * events added with useCapture = false. Model this?
		 */
