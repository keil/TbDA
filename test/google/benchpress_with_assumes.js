...
TreeNode.prototype.check = function () {
  ...
  if (left == null)
    ...
    assumeNonNullUndef("left");
    if ((left.value  <  value) && left.check())
      t1 = true;
  }
  if (t1) {
    if (right == null)
      ...
      assumeNonNullUndef("right");
      if ((right.value  >=  value) && right.check())
        t2 = true;
    }
  }
  return t2;    
...
function isShorter(x, y) {
  var xTail = x, yTail = y;
  while (yTail != null) {
    assumeNonNullUndef("yTail");
    if (xTail == null) return true;
    assumeNonNullUndef("xTail");
    xTail = xTail.next;
    yTail = yTail.next;
  }
  return false;
